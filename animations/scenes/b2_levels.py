...
from lib.geometry import (
    Cl, D5, DELTA, UNIT5, ZETA, in_window, is_cut_level, lens_half_window, lv1, lv2,
    orbit_states, psiL, pt, qs1, window_centre,
)
from lib.style import (
    FAINT, GOOD, HOT, INK, LEFT_C, LENS_C, MUTED, RIGHT_C, Frame, M, T, disk, lens_shape, row,
)
...
class B2Levels(Scene):
    def construct(self):
        ...
        self.wait(2.0)
        steps = VGroup(
            T("one step by a fifth root of unity", 20),
            row("changes", "$\\ell$", "by", "$+3$", "or", "$-2$;", size=20),
        ...
        lensc = VGroup(
            T("To climb (+3) from a left centre, the point", 20),
            T("must lie in the lens of both disks. Projected", 20),
            row("on", "$u$", ", the lens gives", "$|\\psi - Z| \\le D\\sqrt{r^2-1}$", size=20),
            row("(", "$D = |\\mathrm{Im}\\,\\bar u e|$,", "attained at the lens tips)", size=20,
                buff=0.06),
        ...
        slab = row("strip of width", "$2D\\sqrt{r^2-1}$", size=17, color=GOOD).move_to(
        ...
        cutc = VGroup(
            T("A cut level has no tooth inside the strip:", 20),
            T("no left centre on it can climb, so the walk", 20),
            T("never passes it (and likewise from below).", 20),
        ).arrange(DOWN, aligned_edge=LEFT, buff=0.1).next_to(defs, DOWN, aligned_edge=LEFT,
                                                             buff=0.35)
