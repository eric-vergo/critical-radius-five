...
|---|---|---|---|
| `a1_setting` | `A1Setting` | 27 s | The two disks at `r = √(3+φ)`, the turns `a` and `b` (72° clockwise), the lens, the chord `E′E` from rim to rim (`|E+1| = r`), and its three pieces `I₁ = [E′,F′]`, `I₂ = [F′,G′]`, `I₃ = [G′,E]`. |
| `a2_word1` | `A2WordOne` | 22 s | The word `a⁻¹a⁻¹b⁻¹a⁻¹b⁻¹` applied turn by turn: the disk that turns rotates its contents; the piece `I₁` travels through the lens and lands on `[G,F]`, a translation by `2F`. Red rings mark endpoints lying exactly on the rim of the disk about to turn. |
| `a3_word2` | `A3WordTwo` | 22 s | The same for `abab²`: `I₂ → [F,E]`, a translation by `2F`. |
| `a4_word3` | `A4WordThree` | 26 s | The same for `abab⁻¹a⁻¹b⁻¹`: `I₃ → [E′,G]`, a translation by `2F − 2E`. |
| `a5_exchange` | `A5Exchange` | 37 s | The three translations together exchange the pieces (`I₁I₂I₃ → I₃I₁I₂`, lengths `|I₁ ∪ I₂| : |I₃| = 1 : φ`); gluing `E′` to `E` makes this the rotation by `1/φ` of a turn. The orbit of the origin is iterated 300 times and never closes. |
| `b1_lattice` | `B1Lattice` | 36 s | The point's-eye view at `r = 2`: keep the point still and let the pair of disks move. The disk centres sit at lattice points `2ν − 1` and `2(ν + m̄) − 1`, `ν ∈ ℤ[ζ]`; a turn about a centre needs the point within `r` of it (the window). All 143 positions of one orbit, and the same centres under the conjugation `ζ ↦ ζ²` (a bounded cloud). |
| `b2_levels` | `B2Levels` | 42 s | The level `ℓ = −A − 3B` of `ψ = 2⟨u,·⟩ = A + Bφ`, `u = 1 − ζ + ζ²`: rows of teeth `−5q + δℤ` (`δ = 3 − φ`, shifted by `δ/φ` per row), the strip of width `2D√(r² − 1)` given by the lens, the cut levels where no tooth enters the strip, and the orbit's lattice points trapped between them. |
| `b3_threshold` | `B3Threshold` | 31 s | `r` grows through `√(3+φ)`: the strip widens until it is exactly one period wide, the cut levels disappear one by one, and at the same moment the chord `E′E` fits the lens (`E`, `E′` reach the rims). |
| `b4_monotone` | `B4Monotone` | 26 s | Monotonicity: the orbit of `0` at `r = 1, 1.9, 2, 2.05, 2.1, 2.13` (9, 9, 71, 151, 801, 2 543 points). Points are only ever added; at `√(3+φ)` the orbit is infinite. |

## Outputs

* `renders/mp4/*.mp4` — 1920×1080, 60 fps (dark theme).
* `renders/gif/*.gif` — previews, 640 px wide, 12 fps.
* `renders/stills/dark/*.png` — the final frame of each scene (dark theme, for the site).
...
* `lib/style.py` — the two themes, colours, typography, and small drawing helpers.
