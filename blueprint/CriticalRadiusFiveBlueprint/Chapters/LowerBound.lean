...
:::lemma_ "lemma:normal-form" (parent := "lower-bound") (lean := "CriticalRadiusFive.normal_form")
...
:::lemma_ "lemma:typed-walk" (parent := "lower-bound") (lean := "CriticalRadiusFive.edgeClosed")
...
$`0` and dumbbell ends class $`1`. Consequently, if $`V \subseteq \mathbb{Z}[\zeta]` contains $`0` and
$`1` and is closed under $`+\mu_5` from class-$`0` points and under $`-\mu_5` from class-$`1` points,
within the window (for $`v \in V` and $`e \in \mu_5`: $`v + e \in V` if $`\operatorname{Cl}(v) = 0` and
$`v + e \in W_p(r)`, and $`v - e \in V` if $`\operatorname{Cl}(v) = 1` and $`v - e \in W_p(r)`), then
...
:::definition "def:level" (parent := "lower-bound") (lean := "CriticalRadiusFive.lv")
...
:::lemma_ "lemma:step-table" (parent := "lower-bound") (lean := "CriticalRadiusFive.lv_step, CriticalRadiusFive.lv_mod_class")
...
:::lemma_ "lemma:active-step" (parent := "lower-bound") (lean := "CriticalRadiusFive.invariant_breaks_only_at_active")
...
:::lemma_ "lemma:lens-bound" (parent := "lower-bound") (lean := "CriticalRadiusFive.lens_bound")
...
:::lemma_ "lemma:window" (parent := "lower-bound") (lean := "CriticalRadiusFive.psi_class0, CriticalRadiusFive.window")
...
:::lemma_ "lemma:cut-levels" (parent := "lower-bound") (lean := "CriticalRadiusFive.exists_cut_level")
*Cut levels under the golden rotation.* Let $`\delta > 0`, $`0 \le L < \delta/2`, $`\alpha = 1/\varphi`
and $`s \in \{\alpha, -\alpha\}`. Call $`q \in \mathbb{Z}` a _cut level_ for $`Z \in \mathbb{R}` if
...
There is $`N`, depending only on $`\eta`, such that for every $`Z` there are cut levels
$`q_+ \in [1, N]` and $`q_- \in [-N, -2]`; this holds for $`\alpha` and for $`-\alpha` alike, uniformly
in $`Z`.
:::

:::proof "lemma:cut-levels"
Dividing by $`\delta` turns the condition into $`qs - Z/\delta \notin [-\eta, \eta] + \mathbb{Z}`. The
Fibonacci approximations $`F_{m+1}\alpha - F_m = (-1)^m\alpha^{m+1}` provide integers $`k`, of either
sign, for which $`ks` lies within any prescribed $`\varepsilon > 0` of an integer, but not at an
integer. Take $`\varepsilon < 1 - 2\eta` and march through the multiples of such a $`k`: the points
$`jks - Z/\delta` move around $`\mathbb{R}/\mathbb{Z}` in steps shorter than the complementary arc,
which has length $`1 - 2\eta`, so one of a bounded number of them lands in it. The bound depends only
on $`\eta`, not on $`Z`.
:::

:::theorem "thm:confinement" (parent := "lower-bound") (lean := "CriticalRadiusFive.orbit_bounded")
...
two functionals, cut levels $`q_+^{(i)} \in [1, N]` and $`q_-^{(i)} \in [-N, -2]`.

Let $`V` consist of $`0`, $`1` and the lattice points of the window, of class $`0` or $`1`, with
$`5q_-^{(i)} < \operatorname{lv}_i(v) \le 5q_+^{(i)}` for $`i = 1, 2`. By {uses "lemma:active-step"}[],
...
Discreteness. Every $`v \in V` has $`|v| \le 1 + r`, bounded values $`\psi_{u_i}(v)` and bounded
levels $`|\operatorname{lv}_i(v)| \le 5N`. The Galois conjugate of $`\psi_{u_i}(v) = A + B\varphi` is
$`A + B(1 - \varphi) = \varphi^2\psi_{u_i}(v) + \varphi\operatorname{lv}_i(v)`, so it is bounded too.
These are the values at the conjugate point $`\sigma(v)` of two functionals in independent directions,
so $`\sigma(v)` is bounded. Since $`\mathbb{Z}[\zeta]` is discrete in $`\mathbb{C} \times \mathbb{C}`
under $`v \mapsto (v, \sigma(v))`, $`V` lies in a finite set that depends only on $`r` and $`N`, not
on $`p`, and so does the number $`N(r)` of points $`m(p + 1 - 2\nu) - 1` that it produces.
:::

:::theorem "thm:lower" (parent := "lower-bound") (lean := "CriticalRadiusFive.GG_five_finite_of_lt, CriticalRadiusFive.le_criticalRadius_five")
